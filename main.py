...
class TrainingPlanner:
    ...
    def create_widgets(self):
        ...
        self.tree.heading('duration', text='Длительность')        self.tree.grid(row=7, column=0, columnspan=2, padx=10, pady=10)

        # Важно: эта строка должна быть на том же уровне отступа, что и tree.grid(...)
        self.refresh_table()
    ...
    def validate_input(self, date, duration, training_type):
        ...
        # 3. Проверка длительности (положительное число)
        try:
            dur = int(duration)
            if dur <= 0:
                raise ValueError
    ...
    def apply_filter(self):        """Применение фильтров к таблице"""
        try:
    ...
    def load_data(self):
        ...
        if os.path.exists('trainings.json'):
            ...
            except json.JSONDecodeError:                self.trainings = []
            except IOError:
                self.trainings = []
